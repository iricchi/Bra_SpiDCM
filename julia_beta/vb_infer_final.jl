...
function vb_infer(θ0, y_obs, t, n, u_func, use_B; predict_func, prior_var)
    function free_energy(θ)
        ...
        lp = -0.5 * sum(((θ .- prior_mean).^2) ./ prior_var)
    ...
    for _ in 1:8
        ...
        h_diag = diag(FiniteDiff.finite_difference_hessian(θ -> free_energy(θ), θ))
        Σ = inv(Diagonal(-h_diag .+ 10.0))
        θ -= Σ * g
    end
